...
local function score_tooltip(win, id, label, tooltip)
	...
	if player then
		...
		if (me and set.gotboss and set.endtime and PScoresChar and PScoresRaid) then
...
-- solo legacy content role fix
		if (myself == player.name and (not player.role or player.role == "NONE")) then
...
-- saved sets from older Skada versions can have nil role
		...
		--print(player.name,player.role,dt);
		if (dt >= MINIMUMDT) then
			if (not PSMultipliers) then
				PSMultipliers = defaultmultipliers;
			end
			player.score = (player.damage * PSMultipliers.damage[player.role] + player.healing * PSMultipliers.healing[player.role] + mit * PSMultipliers.mitigation[player.role]) / dt;
