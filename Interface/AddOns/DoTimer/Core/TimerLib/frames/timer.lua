...
-- local frameScripts;
-- local counter = 1;
-- local function checkPlayerAuras( frame, showing )
    -- if ( frame.timer.module == AsheylaLib:Import("PlayerAuras") ) then
        -- if showing then
            -- if not frame.protected then
                -- frame.protected = CreateFrame( "Button", "PAFix" .. counter, frame, "SecureActionButtonTemplate" );
                -- local p = frame.protected;
                -- counter = counter + 1;
                -- p:SetFrameLevel( 7 );
                -- p:RegisterForDrag( "LeftButton" );
                -- p:SetAttribute( "unit", "player" );
                -- p:SetAttribute( "type", "cancelaura" );
                -- for k, v in pairs( frameScripts ) do
                    -- if k ~= "OnClick" then
                        -- p:SetScript( k, function( self, ... )
                            -- frame.manager.frames.button:GetScript( k )( frame.manager.frames.button, ... );
                        -- end );
                    -- end
                -- end
            ...
            -- end
            -- frame.protected:SetAttribute( "index", frame.timer.data.buffID );
            -- frame.protected:SetAllPoints( frame );
            -- frame.protected.active = true;
        -- else
            -- if frame.protected then
                -- frame.protected:ClearAllPoints();
                -- frame.protected.active = false;
            -- end
        -- end
    -- end
-- end
...
local internalFuncs = {
    ...
    UpdateShown = function( self )
        if self.timer:IsShown() then
            ...
            -- if self.dead then
                -- self.dead = nil;
                -- checkPlayerAuras( self.frame, true );
            -- end
        else
            self.frame:Hide();
        end
    ...
    TimerDone = function( self, anchor )
        ...
        local timer = self.timer;
        frame:Hide();
        if self.mainFrame.beingDragged then
        ...
        if frame.mouseIsOver then
            if self.tooltips then
                ...
                timer.module:CallScript("OnTimerLeave", timer.group.data, timer.data, timer.id, frame.enteredFrame);
            end
...
local onEnterFunc = function( self )
    ...
    if self:IsMouseOver() then
        timer.module:CallScript("OnTimerEnter", timer.group.data, timer.data, timer.id, self.enteredFrame);
    else
        if self.mouseIsOver and self.tooltips then
            timer.frame:SetScript("OnUpdate", nil);
            timer.module:CallScript("OnTimerLeave", timer.group.data, timer.data, timer.id, self.enteredFrame);
        end
...
frameFuncs = {
    ...
    icon = {
        ...
        drawFrame = function( self, timer, frames )
            ...
            -- this may be being called too early? do testing tomorrow
            -- perhaps self.duration? timer.duration??
            --print( "icon drawframe" );
            local cooldownFrame = frames.cooldown;
            if self.iconShowCooldown and timer.duration then
                cooldownFrame:Show();
                cooldownFrame:SetCooldown(timer.time, timer.duration);
                --print( timer.settings );
                --print( self.iconReverseCooldown );
                cooldownFrame:SetReverse(self.iconReverseCooldown);
                --print( cooldownFrame:GetReverse() );
            else
                --print( "hiding cd" );
                cooldownFrame:Hide();
            end
...
local frameScripts = {
    ...
    OnLeave = function( self )
        local timer = self.timer;
        if self.mainFrame.mouseIsOver and self.manager.tooltips then
            timer.frame:SetScript("OnUpdate", nil);
...
local function acquireTimerFrame( timer, manager )
    ...
    frames.button.mainFrame = frames.mainFrame;
    for k, v in pairs( frameScripts ) do
    ...
    frame.timer = timer;
    -- frame.manager = manager; -- added for PA
