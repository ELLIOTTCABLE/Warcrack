...
function Notifications:Announce(data, ...)
    ...
    elseif data.type == "timer" then
        ...
        self:Timer(msg, data.duration, setting, data.group or self:Get("groupSetting"), data.anchor or self:Get("anchorSetting"), data.texture);
    end
end
...
function Notifications:Timer(msg, duration, setting, group, anchor, texture)
    msg = msg or "Default Msg";
    duration = duration or 5;
    group = group or "Notifications";
    anchor = anchor or "Notifications Anchor";
    local timer = {name = msg, duration = duration, time = GetTime(), texture = texture};
