...
-- protected frames are fucking things up somehow.  this should fix it...
-- sigh, but it doesn't. commenting out for now!
-- local pool = {};
-- local poolFrame = CreateFrame( "Frame" );
-- poolFrame:RegisterEvent( "PLAYER_REGEN_DISABLED" );
-- poolFrame.OnEvent = function( self, event )
    -- for frame in pairs( pool ) do
        -- if frame.active then
            -- frame.active = false;
            -- frame:ClearAllPoints();
            -- frame:SetParent( nil );
            -- if frame:IsDragging() then
                -- frame.frame:GetScript( "OnDragStop" )( frame.frame );
            -- end
            -- frame:Hide();
        -- end
    -- end
-- end
-- local function addToWatched( frame )
...
listener:SetScript("OnTimerEnter", function(self, target, timer, id, frame)
    ...
    --print( "enter:" .. (tostring(frame) or "no frame") .. ":" .. (tostring(frame.protected) or "not here") );
    ...
    repeat
        ...
        -- if target.unit == "player" and target.name == "Buffs" and not InCombatLockdown() then
            -- Ash_GameTooltip:AddLine("|cffff0000" .. "You must right-click"--[[listener.kblib:GetKeyBinding("Timer", "Remove")]] .. "|r to remove.");
        -- end
        if target.unit == "player" and target.name == "Buffs" then 
    ...
    -- if target.unit == "player" and target.name == "Buffs" and not InCombatLockdown() then
        -- if not frame.protected then
            -- frame.protected = CreateFrame( "Button", "PAFix" .. count, nil, "SecureActionButtonTemplate" );
            -- local p = frame.protected;
            -- count = count + 1;
            -- p:RegisterForDrag( "LeftButton" );
            -- p:RegisterForClicks( "AnyUp" );
            -- p:SetAttribute( "unit2", "player" );
            -- p:SetAttribute( "type2", "cancelaura" );
            ...
            -- p:SetScript("OnEvent", function( self, event ) 
                -- if self.active then
                    ...
                    -- if self:IsDragging() then
                        -- self.frame:GetScript( "OnDragStop" )( self.frame );
                    -- end
                    -- self:Hide();
                -- end
            -- end );
            -- p:SetScript( "PreClick", function( self, ... )
                -- self.frame:GetScript( "OnClick" )( self.frame, ... );
            -- end );
            ...
            -- p:SetScript( "OnDragStart", function( self, ... )
                -- self.frame:GetScript( "OnDragStart" )( self.frame, ... );
            -- end );
        ...
        -- if not p.active then
            ...
            -- p:SetPoint( "BOTTOMLEFT", UIParent, "BOTTOMLEFT", frame:GetLeft() * scale, frame:GetBottom() * scale );
            -- p:SetFrameLevel( frame:GetFrameLevel() + 1 );
            -- local t = p:CreateTexture();
            -- t:SetAllPoints( true );
            -- t:SetTexture( 0, 1, 0, .5 );
        -- end
        -- p:SetAttribute( "index2", timer.buffID );
        -- p:Show();
    -- end
end)
...
listener:SetScript("OnTimerLeave", function(self, target, timer, id, frame)
    ...
    --print( "leave:" .. (tostring(frame) or "no frame") .. ":" .. (tostring(frame.protected) or "not here") );
end)
...
listener.kblib:SetClickAction("Timer", "Remove", function(timer, id, target)
    if ( target.name == "Buffs" and target.unit == "player" ) then
        StaticPopup_Show( "PLAYERAURAS2", nil, nil, timer.name );
    end
end)
...
StaticPopupDialogs["PLAYERAURAS2"] = {
	text = "Copy the following text and paste it into your chat frame.",
	button1 = "Done",
	...
	OnShow = function(self, data)
        ...
        self.editBox:SetText( "/cancelaura " .. self.data );
