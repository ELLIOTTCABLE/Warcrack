    ...
    { -- section 4
        ...
        { -- column 2
            ...
            },
            {
                title = "File",
                type = "editBoxString",
                tooltipText = "Type here the name of the sound file you want played.  This file should be found in DoTimer\\Core\\Files.  You can add your own sounds to this folder as well.",
                scripts = {
                    OnChar = function(self)
                        local text = self:GetText();
            ...
            },
            {
                title = "Texture",
                type = "editBoxString",
                tooltipText = "Type here the texture you want for the timer.",
                scripts = {
                    OnChar = function(self)
                        local text = self:GetText();
                        self.table[self.index] = text;
                    end,
                    OnEditFocusGained = function(self) 
                        self:SetText(self.table[self.index]);
                        self:HighlightText(); 
                    end,
                    OnEditFocusLost = function(self)
                        local text = self:GetText();
                        ...
                        if not self.loaded then
                            ...
                            self.Update = function(self)
                                if CURRENT_STATE == "none" or CURRENT_TABLE.type ~= "timer" then
                                    ...
                                    self.table = CURRENT_TABLE;
                                    self.index = "texture";
                                    if not CURRENT_TABLE.texture then
                                        CURRENT_TABLE.texture = "Interface\\Icons\\Spell_Lightning_LightningBolt01";
                                    end
                                    self:Show();
                                    self:SetText(self.title or ""); 
                                    self:SetCursorPosition(0);
        ...
        {
            {
                ...
                func = function()
                    ...
                    elseif type == "timer" then
                        Notifications:Timer(t.msg, t.duration, t.setting, t.group, t.anchor, t.texture);
