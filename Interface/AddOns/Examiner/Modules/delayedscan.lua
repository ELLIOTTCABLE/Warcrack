...
-- Options
ex.options[#ex.options + 1] = { var = "delayedScanFix", default = true, label = "Delayed Gear Scan Fix", tip = "Causes a delayed gear scan of 0.5 seconds, potentially fixing items not showing up on the first inspect" };
...
function mod:OnInspect(unit,guid)
	applyDelay = not ex.isSelf;		-- Do not apply delay if player is inspecting self
end
