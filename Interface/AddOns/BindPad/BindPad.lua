...
function BindPadProfileTab_OnEnter(self, motion)
   ...
   if profileNum == BindPadCore.GetProfileForTalentGroup(1) then
      if profileNum == BindPadCore.GetProfileForTalentGroup(2) then
         GameTooltip:AddLine(format(BINDPAD_TOOLTIP_PROFILE_CURRENTLY2, TALENT_SPEC_PRIMARY, TALENT_SPEC_SECONDARY),
                             0.8, 0.8, 1.0);
      else
         GameTooltip:AddLine(format(BINDPAD_TOOLTIP_PROFILE_CURRENTLY1, TALENT_SPEC_PRIMARY),
                             0.8, 0.8, 1.0);
      end
   elseif profileNum == BindPadCore.GetProfileForTalentGroup(2) then
      GameTooltip:AddLine(format(BINDPAD_TOOLTIP_PROFILE_CURRENTLY1, TALENT_SPEC_SECONDARY), 
                          0.8, 0.8, 1.0);
   end

   local talentGroup = GetActiveSpecGroup();
   if profileNum ~= BindPadCore.GetProfileForTalentGroup(talentGroup) then
      GameTooltip:AddLine(format(BINDPAD_TOOLTIP_PROFILE_CLICK_FOR,
                                 profileNum,
                                 BindPadCore.GetTalentSpec(talentGroup)),
                          0.8, 1.0, 0.8);
   end
...
function BindPadCore.PlaceIntoSlot(id, type, detail, subdetail, spellid)
   ...
   elseif type == "mount" then
      padSlot.type = TYPE_BPMACRO;
      local index = BindPadCore.MountList[detail];
      if index == nil then
	 return;
      end
      if index == 0 then
	 ...
	 local creatureName, spellID, icon, active = C_MountJournal.GetMountInfo(index);
...
function BindPadCore.InitProfile()
   ...
   local newActiveTalentGroup = GetActiveSpecGroup();
...
function BindPadCore.GetSpecTexture(talentGroup)
   ...
   if nil == specInfoCache then
      ...
      local primaryTree = GetSpecialization(false, false, talentGroup);
      if ( primaryTree and primaryTree > 0 ) then
         -- the spec had a primary tab
         return specInfoCache[primaryTree].icon;
      else
...
function BindPadCore.GetTalentSpec(talentGroup)
...
function BindPadCore.GetMacroIconInfo(index)
   ...
   if (texture) then
	 ...
	 BindPadCore.HiddenSlot.icon:SetToFileData(texture);
...
--   end
   ...
   -- Actually no need to save bindings here because BindPad will restore all keybindings every login.
   --  BindPadCore.SaveBindings(GetCurrentBindingSet());
...
BindPadCore.MountList = {};
function BindPadCore.MountJournal_PickupHook(index)
   local type, detail = GetCursorInfo();
   if type == nil then
      return;
   end
   BindPadCore.MountList[detail] = index;
end
hooksecurefunc(C_MountJournal, "Pickup", BindPadCore.MountJournal_PickupHook);
