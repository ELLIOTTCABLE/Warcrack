    ...
    toggle = {
        ...
        OnClick = function(self) 
            if self:GetChecked() then
                ...
                alertUser("You cannot check this button.  It becomes checked when you change the setting to the right.");
            else
                if self.Set then
                    self.Set(self.setting, nil);
                else
...
function gui:GenerateItem(module, frame, item, extras)
    ...
    elseif item.type == "editBoxDropDown" then
        ...
        local func = item.func or function()
            local info;
            local t = extras.get and extras.get(item.setting) or module:Get(item.setting);
            if ( t == nil ) then
                print( "FAIL: " .. item.setting );
            end
...
function gui:ShowPanel(panel, subCat)
    local old = InterfaceOptionsFrame.lastFrame;
    ...
    InterfaceOptionsFrame.lastFrame = old;
end
...
function ensureShown( panel )
    ...
    for i, element in ipairs( INTERFACEOPTIONS_ADDONCATEGORIES ) do
    ...
    if ( elementToDisplay.hidden and elementToDisplay.parent ) then 
        local button = ensureShown( elementToDisplay.parent );
    ...
    local count = 0;
    for i, e in ipairs( INTERFACEOPTIONS_ADDONCATEGORIES ) do
        if e == elementToDisplay then
            InterfaceOptionsFrameAddOnsListScrollBar:SetValue( count * InterfaceOptionsFrameAddOns.buttonHeight );
            break;
        end
...
hooksecurefunc( "InterfaceOptionsFrame_OpenToCategory", function( panel, suppress )
    ...
    ensureShown( panel or panelName );
    InterfaceOptionsFrame_OpenToCategory( panel or panelName, true );
end );
