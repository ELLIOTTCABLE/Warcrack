...
local function OnInitialize(plate)
	...
	if db.debuffWidget.ON then
		if not w.WidgetDebuff then
			local widget
			widget = TidyPlatesWidgets.CreateAuraWidget(plate)
	...
	if db.socialWidget.ON then
		if not w.SocialArt then 
			local widget
			widget = ThreatPlatesWidgets.CreateSocialWidget(plate)
	...
	if db.totemWidget.ON then
		if not w.TotemIconWidget then 
			local widget
			widget = ThreatPlatesWidgets.CreateTotemIconWidget(plate)
	...
	if db.uniqueWidget.ON then
		if not w.UniqueIconWidget then 
			local widget
			widget = ThreatPlatesWidgets.CreateUniqueIconWidget(plate)
	...
	if db.targetWidget.ON then
		if not w.TargetArt then
			local widget
			widget = ThreatPlatesWidgets.CreateTargetFrameArt(plate)
	...
	-- Elite Icon Widget
	--[[if db.eliteWidget.ON then
		if not w.EliteArt then
			local widget
			widget = ThreatPlatesWidgets.CreateEliteFrameArt(plate)
			widget:SetHeight(db.eliteWidget.scale)
			widget:SetWidth(db.eliteWidget.scale)
			widget:SetPoint("CENTER",plate,(db.eliteWidget.anchor), (db.eliteWidget.x), (db.eliteWidget.y))
			w.EliteArt = widget
		end
	elseif w.EliteArt then
		w.EliteArt:Hide()
		w.EliteArt = nil	
	end]]--
	-- Class Icon Widget 
	...
	if db.settings.elitehealthborder.show then
		if not w.EliteOverlay then
			local widget 
			widget = ThreatPlatesWidgets.CreateEliteFrameArtOverlay(plate)
			widget:SetPoint("CENTER", plate, "CENTER", 0, 0)
	...
	if db.threat.art.ON and db.threat.ON then
		if not w.ThreatArtWidget then 
			local widget
			widget = ThreatPlatesWidgets.CreateThreatArtWidget(plate)
			widget:SetPoint("CENTER", plate, "CENTER", 0, 0)
	...
	if db.threatWidget.ON then
		if not w.ThreatLineWidget then
			local widget
			widget = TidyPlatesWidgets.CreateThreatLineWidget(plate)
	...
	if db.tankedWidget.ON and TidyPlatesThreat.db.char.threat.tanking then
		if not w.TankedWidget then
			local widget
			widget = TidyPlatesWidgets.CreateTankedWidget(plate)
			widget:SetHeight(db.tankedWidget.scale)
			widget:SetWidth(db.tankedWidget.scale)
			widget:SetPoint("CENTER", plate, db.tankedWidget.x, db.tankedWidget.y)
			w.TankedWidget = widget			
		end
	...
	if db.comboWidget.ON then
		if not w.ComboPoints then 
			local widget
			widget = ThreatPlatesWidgets.CreateComboPointWidget(plate)
...
local function OnContextUpdate(plate, unit)
	...
	-- Debuff Widget
	if db.debuffWidget.ON then
		if not w.WidgetDebuff then OnInitialize(plate) end
	...
	-- Combo Point Widget
	if db.comboWidget.ON then
		if not w.ComboPoints then OnInitialize(plate) end
	...
	if db.threatWidget.ON and unit.class == "UNKNOWN" then
		if not w.ThreatLineWidget then OnInitialize(plate) end
...
local function OnUpdate(plate, unit)
	...
	-- Target Art
	if db.targetWidget.ON then
		if not w.TargetArt then OnInitialize(plate) end
		w.TargetArt:Update(unit)
	end
	-- Elite Overlay
	if db.settings.elitehealthborder.show then
		if not w.EliteOverlay then OnInitialize(plate) end
		w.EliteOverlay:Update(unit)
	end
	-- Elite Icon
	--[[if db.eliteWidget.ON then
		if not w.EliteArt then OnInitialize(plate) end
		w.EliteArt:SetHeight(db.eliteWidget.scale)
		w.EliteArt:SetWidth(db.eliteWidget.scale)
		w.EliteArt:SetPoint("CENTER",plate,(db.eliteWidget.anchor), (db.eliteWidget.x), (db.eliteWidget.y))
		w.EliteArt:Update(unit)
	end]]--
	-- Social Widget Textures
	if db.socialWidget.ON then
		if not w.SocialArt then OnInitialize(plate) end
	...
	-- Class Icons
	if db.classWidget.ON then
		if not w.ClassIconWidget then OnInitialize(plate) end
	...
	-- Totem Icons
	if db.totemWidget.ON then
		if not w.TotemIconWidget then OnInitialize(plate) end
	...
	-- Unique Icons
	if db.uniqueWidget.ON then
		if not w.UniqueIconWidget then OnInitialize(plate) end
	...
	if db.threat.ON and db.threat.art.ON then
		if not w.ThreatArtWidget then OnInitialize(plate) end
