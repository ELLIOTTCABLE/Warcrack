...
local GetSpellBookItemName, GetSpellTexture, BOOKTYPE_SPELL, BOOKTYPE_PET, GetContainerNumSlots, GetItemInfo, GetContainerItemInfo, 
    GetItemSpell, GetContainerItemLink, GetInventoryItemLink, GetInventoryItemTexture, GetActionTexture, GetActionInfo = 
    GetSpellBookItemName, GetSpellTexture, BOOKTYPE_SPELL, BOOKTYPE_PET, GetContainerNumSlots, GetItemInfo, GetContainerItemInfo, 
    GetItemSpell, GetContainerItemLink, GetInventoryItemLink, GetInventoryItemTexture, GetActionTexture, GetActionInfo;

function SpellLib:GetSpellInfo(i,book,side,index)
    SpellSystemScanningFrame:SetOwner(UIParent,"ANCHOR_NONE")
    SpellSystemScanningFrame:SetSpellBookItem(i,book)
    ...
    if type then
        if type == "spell" then
            ...
            while name do
                ...
                i = i + 1
                name,rank = GetSpellBookItemName(i,BOOKTYPE_SPELL)
            end
            i = 1
            name,rank = GetSpellBookItemName(i,BOOKTYPE_PET)
            while name do
                if name == query then 
