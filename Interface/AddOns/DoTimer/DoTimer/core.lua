...
DoTimer.HELPFUL_TYPE = "HELPFUL|PLAYER";
DoTimer.HARMFUL_TYPE = "HARMFUL|PLAYER";
...
local GetSpellBookItemName, GetSpellTexture, UnitExists, UnitIsUnit, UnitCanAttack, 
    ...
    GetSpellBookItemName, GetSpellTexture, UnitExists, UnitIsUnit, UnitCanAttack, 
    UnitIsGhost, UnitAffectingCombat, GetRaidTargetIndex, UnitDebuff, UnitBuff, 
    UnitChannelInfo, UnitIsPlayer, UnitPlayerControlled, UnitGUID;
...
function getEnglish(spellname)
    ...
    for index,value in ipairs(spellbooktables) do
        local i = 1
        while GetSpellBookItemName(i, value) do
            local spell = GetSpellBookItemName(i, value);
...
local combatFunc = function(self, elapsed)
    ...
    if (self.elapsed >= 3) then
        if not UnitAffectingCombat("player") then
            for i, target in ipairs(targets) do
                if target.type == "mob" and not string.find(target.name, "Dummy") then
                    target:RemoveAllTimers();
                end
            end
        end
...
function DoTimer:UNIT_SPELLCAST_SUCCEEDED(...)
    local unit, spellName, spellRank, _, spellID = ...;
    ...
    if self:IsAOESpell(spellName) then
        ...
        local duration = self:ReturnDuration(spellID, texture);
            ...
            if (not self:IsIgnoring(name, rank, texture, count, duration, endTime, filter, guid, unitName, unit, isMine)) then
                ...
                if target then
                    for _i, timer in ipairs(target.timers) do
                        if (timer.module == self and timer.name == name and timer.type == filter and not timer.validated) then
                ...
                if (adding) then
                    if uniqueSpells[self:ReturnEnglish(name)] then
...
setmetatable(cachedDurations, {
    __index = function(t, k)
        local dur = getDuration(k);
        t[k] = dur;
        return dur;
    end
})
function DoTimer:ReturnDuration(spellID, texture) --returns the duration of a spell
    ...
    if not localeData[texture] then return 0; end
    return cachedDurations[spellID];
end

function getDuration(spellID)
    local texture = GetSpellTexture( spellID );
    local spellName = GetSpellInfo( spellID );
    for index, value in ipairs(spellbooktables) do 
        local i = 1;
        while GetSpellBookItemName(i,value) do
            local spellname = GetSpellBookItemName(i,value);
