		...
		if (noPopup) then
			...
			if (not frame.popupHandler) then
				...
				SecureHandlerWrapScript(frame, "OnEnter", popupHandler, [[
					popupHeader = self:GetFrameRef("popupHeader")
					popupHeader:Show()
					popupHeader:RegisterAutoHide(0.25)  -- Register auto hide                         
					popupHeader:AddToAutoHide(self)  -- Add the frame to the check area
										
					local children = table.new()
					popupHeader:GetChildList(children)
					for i=1, #children do
						popupHeader:AddToAutoHide(children[i]) -- Add children to the check area
					end
					
			]])
			SecureHandlerWrapScript(frame, "OnAttributeChanged", popupHandler, [[
