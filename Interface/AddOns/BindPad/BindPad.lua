...
function BindPadSlot_OnEnter(self)
   ...
   elseif TYPE_SPELL == padSlot.type then
      ...
      if spellID then
	 GameTooltip:SetSpellBookItem(spellID, padSlot.bookType)
      else
...
function BindPadProfileTab_OnShow(self)
   ...
   local subIcon = _G[self:GetName().."SubIcon"];
...
function BindPadCore.PlaceIntoSlot(id, type, detail, subdetail)
   ...
   elseif type == "spell" then
      ...
      if BindPadCore.IsSingleRank(spellName, subdetail) then
	 padSlot.rank = nil;
      else
...
function BindPadCore.PickupSlot(self, id)
   ...
   elseif TYPE_SPELL == padSlot.type then
      local spellID = BindPadCore.FindSpellIdByName(padSlot.name, padSlot.rank, padSlot.bookType);
      if spellID then
	 PickupSpellBookItem(spellID, padSlot.bookType);
      end
   elseif TYPE_MACRO == padSlot.type then
...
function BindPadCore.InitCash()
   for k,v in pairs(BindPadCore.IsSingleCash) do
      BindPadCore.IsSingleCash[k] = nil;
   end
   
end

function BindPadCore.IsSingleRank(spellName, bookType)
   local cash = BindPadCore.IsSingleCash[spellName];
   if cash ~= nil then
      return cash;
   end
   ...
   for i = BindPadCore.GetSpellNum(bookType), 1, -1 do
      local ispellName, ispellRank = GetSpellBookItemName(i, bookType);
   ...
   local result;
   if count > 1 then
      -- There are multiple ranks of same spell.
      result = false;
   else
      -- There is only single rank of this spell.
      result = true;
   end
   
   BindPadCore.IsSingleCash[spellName] = result;
   return result;
end
...
function BindPadCore.GetActionCommand(actionSlot)
   ...
   if type == "spell" then
	 ...
	 return nil;
      ...
      if BindPadCore.IsSingleRank(spellName, subType) then
	 spellRank = nil;
      end
