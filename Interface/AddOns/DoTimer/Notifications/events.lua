...
local blizzEvents = {
    ...
    ["COMBAT_TEXT_UPDATE"] = {"Spell Active"},
};
...
function Notifications:COMBAT_LOG_EVENT_UNFILTERED(timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, ...)
        ...
        if reaction == COMBATLOG_OBJECT_REACTION_FRIENDLY then
            ...
            if destGUID == UnitGUID("player") then
                if auraType == "BUFF" then
                    self:FireEvent("Buff Gain", spellName, destName);
                    AsheylaLib:CallEvent("NOTIFICATIONS_BUFF_GAIN", spellName);
                elseif auraType == "DEBUFF" then
                    self:FireEvent("Debuff Gain", spellName, destName);
                    AsheylaLib:CallEvent("NOTIFICATIONS_DEBUFF_GAIN", spellName);
                end
            elseif affiliation ~= COMBATLOG_OBJECT_AFFILIATION_OUTSIDER and destGUID ~= UnitGUID("player") then
                if auraType == "BUFF" then
    ...
    elseif event == "SPELL_AURA_REMOVED" then
        ...
        if reaction == COMBATLOG_OBJECT_REACTION_FRIENDLY then
            ...
            if destGUID == UnitGUID("player") then
                if auraType == "BUFF" then
                    self:FireEvent("Buff Fade", spellName, destName);
                    AsheylaLib:CallEvent("NOTIFICATIONS_BUFF_FADE", spellName);
                elseif auraType == "DEBUFF" then
                    self:FireEvent("Debuff Fade", spellName, destName);
                    AsheylaLib:CallEvent("NOTIFICATIONS_DEBUFF_FADE", spellName);
                end
            elseif affiliation ~= COMBATLOG_OBJECT_AFFILIATION_OUTSIDER and destGUID ~= UnitGUID("player") then
                if auraType == "BUFF" then
