    ...
    repeat
        ...
        if timer.foundTooltip then
            if timer.tooltipType == "spell" then
                Ash_GameTooltip:SetSpellBookItem(timer.tooltipIndex, timer.tooltipBook);
            else
...
f:SetScript( "OnEvent", function( self, _, timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellID, spellName )
    ...
    if event == "SPELL_AURA_APPLIED" or event == "SPELL_PERIODIC_DAMAGE" then
        local time = GetTime();
        dbg_print( ("saw tick for %s: %.2f"):format( spellName, time ) );
        ...
        if seen[ -1 ] then
            dbg_print( ("error in guess: %.2f"):format( math.abs( time - seen[ -1 ] ) ) );
        end
        ...
        if n > 1 then
            ...
            seen[ -1 ] = time + elapsed;
            dbg_print( ("actual elapsed time: %.2f"):format( seen[ n ] - seen[ n - 1 ] ) );
            dbg_print( ("average elapsed time: %.2f"):format( elapsed ) );
            dbg_print( ("predicted tick for %s: %.2f"):format( spellName, time + elapsed ) );
        end
    end
end );--]]
