...
do
	...
	function Ellipsis:PopulatePlayerSpellCooldowns()
		...
		if (select(2, UnitClass('player')) == 'PRIEST') then -- special case to fix priest's cascade
			local name = GetSpellInfo(121135) -- get localized name for Cascade)

			if (spellCooldowns[name]) then
				if (GetSpecialization() == 3) then -- shadow
					spellCooldowns[name].spellID = 127632
					spellCooldowns[name].icon = GetSpellTexture(127632)
				else -- holy or disc
					spellCooldowns[name].spellID = 121135
					spellCooldowns[name].icon = GetSpellTexture(121135)
				end
			end
		end
